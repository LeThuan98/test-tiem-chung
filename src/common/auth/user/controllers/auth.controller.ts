...
export class AuthController {
    ...
    async login(@Body() dto: LoginDto): Promise<any> {
        ...
        return this.response.detailSuccess(
            this.transformer.transformUserDetail(login, { token: login.token }),
        );
    }
    ...
    async logout(@Request() req: Record<string, any>): Promise<any> {
        ...
        let logout = await this.authService.logout({
            token,
    ...
    async getProfile(@UserAuth() user: User): Promise<any> {
        let profile = await this.authService.getProfile(user.id);
    ...
    async postProfile(
        ...
        let profile = await this.authService.postProfile(user.id, dto, files);
