...
export class UserAuthService {
    ...
    async isAdmin(token: string): Promise<boolean> {
        ...
        let userCreated = await this.usersService.detail(user.user.id);
    ...
    async login(data: Object): Promise<any> {
        ...
        let item = await this.usersService.findOne({ email: email, active: true });
    ...
    async postProfile(id: string, data: Object, files: Record<any, any>): Promise<any> {
        ...
        let item = await this.usersService.findOne({ _id: id, active: true });
        ...
        if (typeof password != 'undefined')
            comparedResult = await this.helperService.compareHash(currentPassword, item.password);
        if (!comparedResult)
            return this.helperService.throwException('The current password is in-correct.');
    ...
    async verifyToken(token: string): Promise<any> {
        ...
        let user = await this.usersService.findOne({ _id: payload.id, active: true });
        if (!user)
            return this.helperService.throwException('Token is in-valid', HttpStatus.FORBIDDEN);
        return { user, payload };
    }
    ...
    private async signToken(user: User): Promise<any> {
        let expireAtArr =  this.configService.get('JWT_USER_EXPIRE').split(/(\d+)/).filter(Boolean);
