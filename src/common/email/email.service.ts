...
export class EmailService {
    ...
    public async sendMail(mailto: string, subject: string, template: string, data: Record<any, any>, isVerify: boolean): Promise<boolean> {
        if (isVerify) {
    ...
    public async verifyEmail(email: string): Promise<boolean> {
        ...
        const result = await this.httpService.get<any>(`https://api.millionverifier.com/api/v3/?api=${key}&email=${email}`).toPromise();
        return result.data.resultcode == 1;
    }

    public async sendForgotPassword(data: {code: number, email: string, name: string}) {
        await this.sendMail(data.email, '[TB] Quên mật khẩu', 'forgotPassword', {
