...
export interface compressOptionInterface { jpg: number, png: number }
...
export async function compressImage(srcImage: string, destDir: string, compressOptions: compressOptionInterface = {
    jpg: 75, png: 75,
}) {
...
export async function resizeImage(src: string, dest: string, width: number, height: number, fitOption = 'inside') {
    return await sharp(src).resize(width, height, {
        fit: fitOption,
    }).toFile(dest);
}
...
export function thumb(doc: any, field: string, collectionName: string, scales?: object, type?: string): string {
    ...
    if (configUpload().FILESYSTEM_DRIVER == 'public') {
        var dirUrl = `${configUpload().NODE_URL}/${configUpload().PREFIX_UPLOAD_URL}/${collectionName}/${doc.id ? doc.id : doc._id}/${field}`;
    } else if (configUpload().FILESYSTEM_DRIVER == 'do') {
        var dirUrl = `${configUpload().DO_URL}/${configUpload().PREFIX_UPLOAD_URL}/${collectionName}/${doc.id ? doc.id : doc._id}/${field}`;
    } else if (configUpload().FILESYSTEM_DRIVER == 'gc') {
        var dirUrl = `${configUpload().GC_URL}/${configUpload().PREFIX_UPLOAD_URL}/${collectionName}/${doc.id ? doc.id : doc._id}/${field}`;
    }
...
export async function saveThumbTrans(doc: any, field: string, collectionName: string, scales?: object, fitOption?: string, compressOptions?: compressOptionInterface) {
    const fileNameTrans = doc[field];
    await Promise.all(Object.keys(fileNameTrans).map(async function(locale) {
...
export async function downloadThumb(res: any, doc: any, field: string, collectionName: string, scales?: object, fitOption?: string): Promise<{status: Boolean, res?: any}> {
    var fileName = doc[field];
    var dirFolder = `${collectionName}/${doc.id}/${field}`;
    if(configUpload().FILESYSTEM_DRIVER == 'public') {
        if(fileName == 'null') {
            return {
                status: false
        ...
        if(fileName == 'null') {
            ...
            return await s3.getObject(params).promise().then(async (data) => {
                let timestamp = Date.now();
                let baseExt = extname(fileName);
                let randomNameTmp = `${configUpload().PREFIX_UPLOAD_TMP}/${timestamp}${randStr(5)}${baseExt}`;
...
export async function saveFileLocal(src: string, dirDest: string, scales?: object, fitOption?: string, compressOptions?: compressOptionInterface) {
    ...
    if (isImg && scales && Object.keys(scales).length > 0) {
        await Promise.all(Object.keys(scales).map(async function(index) {
...
export async function getAllLinkFileDO() {
    ...
    const data = await s3.listObjects(params).promise();      
    let files = data['Contents'] as Array<Object>
    let fileURLs = []
