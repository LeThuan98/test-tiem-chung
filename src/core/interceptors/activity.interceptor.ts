...
export class ActivityInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        // const response = context.switchToHttp().getResponse();
            ...
            const methodNameVi = request.method == 'POST' ? 'tạo' : request.method == 'PUT' ? 'cập nhật' : request.method == 'DELETE' ? 'xoá' : '';
            const methodNameEn = request.method == 'POST' ? 'created' : request.method == 'PUT' ? 'updated' : request.method == 'DELETE' ? 'deleted' : '';
            return next.handle().pipe(tap((d) => {
                data.body = {...request.body};
                nameActivity = data.body.name ? data.body.name : data.body.ids ? JSON.stringify(data.body.ids) : '';
